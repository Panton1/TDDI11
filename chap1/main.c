...
{	
	...
	unsigned const int low_speed=1;
	unsigned const int medium_speed=2;
	unsigned const int full_speed=3;
	...
	while(true)
	{
		...
		isAcOn = ((inputRegister & (1<<4)) >0);  // Assignment 1<<4 flyttar VL fyra steg åt pils riktning

		
		// bits 5 to 9 indicate desired temperature
		unsigned int temperatureDesired_tmp = 0; // Assignment
		temperatureDesired_tmp=inputRegister>>5; // assignment
		temperatureDesired_tmp&=0x0000001f;  //assignement

		//printf("temperatureDesired_tmp casted to float=%f\n",(float)temperatureDesired_tmp); //for debug
		// According to equation 1:
		temperatureDesired = 14.50+temperatureDesired_tmp*0.5;  // Assignment
		...
		temperatureMeasured_fractional = inputRegister>>10; // Assignment
		...
		// Assignment
		// Assignment
		temperatureMeasured = temperatureMeasured_decimal+temperatureMeasured_fractional*0.2;
		
		if((temperatureMeasured_decimal==0x0000001f) && (temperatureMeasured_fractional==0))
		  {
		    temperatureMeasured=-1;
		  }
		if ((temperatureMeasured_decimal==0x0000001f) && (temperatureMeasured_fractional==7))
		  {
		...
		if(fanSpeedSelector == 0x00000000)
			fanSpeed = full_speed; // Assignment
		else if (fanSpeedSelector == 0x00000005) // Assignment
			fanSpeed = medium_speed; // Assignment
		else if (fanSpeedSelector == 0x00000003) // Assignment
			fanSpeed = low_speed; // Assignment
		else if (fanSpeedSelector == 0x00000007)
			fanSpeed = off; // Assignment
		...
		if (isAcOn)
		{
			...
			if((temperatureMeasured - temperatureDesired+ Hysteresis_Heater) < 0)
			  {
			    isCoolerOn = false;
			    isHeaterOn = true;
			  }
		...
		//if heater är igång och mätt temp är högre än önskad temp
