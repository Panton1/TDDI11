...
char liuids[30] = "antpe029 and noegi438";
...
static uint8_t Check_Move_Validity(uint16_t x, uint16_t y,
		Cell_State BoardState[], uint8_t *p_choice_i, uint8_t *p_choice_j);
...
int main(void) {
	...
	while (1) {
		...
		{ // Read touch and Button. Do not modify.
			if (TouchDetected == SET) { // Detects screen touch (PersistentTouch)
				TouchDetected = RESET;
			...
			if (UserButtonPressed == SET) { // Detects button pressure (PersistentButton).
		...
		if (State != PreviousState) {
			...
			case BLUE_UPDATE:
				...
				break;
			case BLUE_WINNER:
				Display_Text((uint8_t*) "Blue won the game!!");
				Display_Other_Text((uint8_t*) "Touch screen to start new game");
				break;
			case RED_QUESTION:
				Touch = 0;
				Button = 0;
			...
			case RED_ANSWER:
				...
				break;
			case RED_WINNER:
				Display_Text((uint8_t*) "Red won the game!!");
				Display_Other_Text((uint8_t*) "Touch screen to start new game");
				break;
			case NO_WIN:
				Clear_Display_Board(BoardState);
				Display_Text((uint8_t*) "DRAW!!");
				Display_Other_Text((uint8_t*) "Touch screen to start new game");
				break;

			case EXIT:
				Clear_Display_Board(BoardState);
				Display_Text((uint8_t*) "Antpe029 & NOEGI438 :)");
				Display_Other_Text((uint8_t*) "Good bye");
				break;
			default:
				Clear_Display_Welcome();
...
static void Clear_Display_Board(Cell_State BoardState[]) {
	...
	for (int i = 0; i < 3; i++) {
		for (int j = 0; j < 3; j++) {
			...
			case BLUE:
				UTIL_LCD_FillCircle(XPOS(i), YPOS(j), RADIUS - 3,
						UTIL_LCD_COLOR_BLUE);
				break;
			case RED:
				UTIL_LCD_FillCircle(XPOS(i), YPOS(j), RADIUS - 3,
				...
				break;
			default:
				UTIL_LCD_FillCircle(XPOS(i), YPOS(j), RADIUS - 2,
...
static uint8_t Check_Move_Validity(uint16_t x, uint16_t y,
	...
	for (int i = 0; i < 3; i++) {
		for (int j = 0; j < 3; j++) {
			if ((y > (YPOS(j) - RADIUS)) && (y < (YPOS(j) + RADIUS))) {

				if ((x > (XPOS(i) - RADIUS)) && (x < (XPOS(i) + RADIUS))) {
...
static uint8_t Check_Win(Cell_State BoardState[]) {
	...
	if (BoardState[1 + 3 * 1] != FREE) {
		for (int i = 0; i < 4; i++) {
			if ((BoardState[i] == BoardState[4])
					&& (BoardState[4] == BoardState[8 - i])) {
				return 1;
			}
		}
	}
	...
	if (BoardState[8] != FREE) {
		if ((BoardState[8] == BoardState[7])
				&& (BoardState[8] == BoardState[6])) {
			return 1;
		}
		if ((BoardState[8] == BoardState[5])
...
static void SystemHardwareInit(void) {
	...
	if (ButtonInitialized != SET) {
		if (TicTacToe_PB_Init(BUTTON_USER,
				BUTTON_MODE_EXTI) != TicTacToe_ERROR_NONE) {
			Error_Handler();
		}
	...
	if (LcdInitialized != SET) {
		...
		/* Initialize the LCD */
		if (TicTacToe_LCD_Init(0,
				LCD_ORIENTATION_PORTRAIT) != TicTacToe_ERROR_NONE) {
			Error_Handler();
		}
